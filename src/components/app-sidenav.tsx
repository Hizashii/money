...
export default function AppSidenav() {
  ...
  return (
    <>
      ...
      <aside
        className={`lg:hidden fixed top-0 right-0 z-50 h-full w-full max-w-[280px] bg-white border-l border-slate-200 shadow-xl flex flex-col transition-transform duration-200 ease-out ${
      ...
      >
        <div className="h-14 px-4 flex items-center justify-between border-b border-slate-200">
          ...
          <button
            type="button"
            onClick={() => setMobileOpen(false)}
            className="p-2 -mr-2 rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-900 transition"
        ...
        <nav className="flex-1 p-4 overflow-auto" aria-label="App navigation">
          <NavLinks pathname={pathname} onLinkClick={() => setMobileOpen(false)} />
        </nav>
        <div className="p-4 border-t border-slate-200">
          <p className="px-3 text-xs text-slate-400">Invoice extraction tool</p>
        </div>
      </aside>
